 ...
 */
function contentDataExtractor(): Function {
  const known = []
 ...
 * @returns {string}
 */
function compactWS(str: string): string {
 ...
 */
function stylelessClone(el: SVGSVGElement): SVGSVGElement {
  const clone = <SVGSVGElement>el.cloneNode(true)
  clone.removeAttribute('style')
  clone.removeAttribute('class')
  for (let el of Array.from(clone.querySelectorAll('style'))) {
    el.remove()
  }
...
function getSymbols(root: HTMLElement) {
 ...
 */
function getExternalUrls(root: HTMLElement, forceUpdate: boolean): string[] {
  const nodes: SVGUseElement[] = Array.from(root.querySelectorAll('svg use'))
  return nodes
    .map(el => {
 ...
 * we're caching
 */
function lookUp(forceUpdate: boolean) {
  // Current document
  sendResult({
    type: 'symbol',
    location: THIS_URL,
    action: () => getSymbols(document.documentElement),
    forceUpdate: forceUpdate
  })
  sendResult({
    type: 'svg',
    location: THIS_URL,
    action: () => getVisibleSvg(document.documentElement),
    forceUpdate: forceUpdate
  })

  // External docs
  getExternalUrls(document.documentElement, forceUpdate).forEach(url => {
    fetch(url)
      .then(resp => resp.text())
      .then(svgText => {
        ...
        if (svgDoc.documentElement.nodeName === 'svg') {
          sendResult({
            type: 'symbol',
            location: url,
            action: () => getSymbols(svgDoc.rootElement),
            forceUpdate: false
          })
        }
      })
